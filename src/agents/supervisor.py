...
class Supervisor:
    ...
    def judge_quality(self, state: GraphState) -> Dict[str, Any]:
        """
        Judge Loop: Evaluate the quality and coverage of collected data.
        """
        ...
        coverage_score = 0
        if candidates: coverage_score += 0.3
        if tech and len(tech) >= len(candidates): coverage_score += 0.3
        if market and len(market) >= len(candidates): coverage_score += 0.4
    ...
    def __call__(self, state: GraphState):
        ...
        if current_iter >= 5: # Max iterations
            print("MAX ITERATIONS REACHED. FORCING REPORT GENERATION.")
        ...
        print(f"Supervisor Plan: {result.plan}")
